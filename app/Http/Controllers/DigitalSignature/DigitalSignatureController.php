...
class DigitalSignatureController extends Controller
{
    ...
    public function processDocumentSigning(Request $request, $approvalRequestId)
    {
        ...
        try {
            ...
            $result = DB::transaction(function () use (
                ...
                // ========================================================================
                // STEP 3 & 4 COMBINED: Generate Digital Signature Key FIRST
                // ========================================================================
                // Generate unique digital signature key for this document
                $digitalSignature = $this->digitalSignatureService->createDigitalSignatureForDocument($documentSignature);

                Log::info('Digital signature key generated', [
                    'digital_signature_id' => $digitalSignature->id,
                    'signature_id' => $digitalSignature->signature_id
                ]);

                // ========================================================================
                // STEP 5: ONE-PASS: Embed QR Code + Sign PDF Simultaneously
                // NEW: This replaces old 2-pass flow (embed QR → sign separately)
                // ========================================================================
                $pdfWithQRPath = $this->pdfSignatureService->embedQRCodeAndSignPDF(
                    ...
                    [
                        ...
                        'signer_name' => $approvalRequest->approver->name ?? 'Unknown Signer / System Generated',
                        // 'reason' => 'Document Approval',
                        'reason' => 'Internally Signed Document for ' . $approvalRequest->document_name . ' by ' . ($approvalRequest->approver->name ?? 'System Generated'),
                        'location' => 'Universitas Muhammadiyah Tangerang',
                        'contact_info' => $approvalRequest->approver->email ?? 'ft@umt.ac.id'
                    ]
                );

                Log::info('QR code embedded and PDF signed (ONE-PASS method)', [
                    'original_pdf' => $approvalRequest->document_path,
                    'signed_pdf' => $pdfWithQRPath,
                    'digital_signature_id' => $digitalSignature->id,
                    'adobe_reader_compatible' => true
                ]);

                // ========================================================================
                // STEP 6: Create CMS Signature and Update DocumentSignature
                // ========================================================================
                ...
                $documentSignature->update([
                    ...
                    'signature_format' => $signatureData['signature_format'],
                    'signed_at' => $signatureData['signed_at'],
                    'signed_by' => $approvalRequest->approved_by,
                    'signature_status' => DocumentSignature::STATUS_VERIFIED,
                    'signature_metadata' => array_merge($signatureData['metadata'] ?? [], [
                        'signing_method' => 'one_pass_tcpdf',
                        'adobe_reader_compatible' => true
                    ]),
                    'final_pdf_path' => $pdfWithQRPath
                ]);

                $signedDocumentSignature = $documentSignature->fresh(['digitalSignature', 'approvalRequest']);

                Log::info('Document signature updated with CMS signature', [
                ...
                // ========================================================================
                // STEP 7: Update DocumentSignature with Additional Metadata
                // ========================================================================
                ...
                // ========================================================================
                // STEP 8: Update Approval Request Status (Auto-Verify)
                // ========================================================================
    ...
    //! BACKUP processDocumentSigning Before Add Signature to PDF - UNUSED METHODS BELOW
    // public function processDocumentSigning(Request $request, $approvalRequestId)
    // {
    //     $startTime = microtime(true); // Track signing duration
