{{-- resources/views/digital-signature/admin/logs/verification.blade.php --}}
@extends('digital-signature.layouts.app')

@section('title', 'Verification Logs - Activity Logs')
...
<div class="main-content">
    ...
    <ul class="nav nav-tabs mb-4" role="tablist">
        <li class="nav-item" role="presentation">
            <a class="nav-link" href="{{ route('admin.signature.logs.audit') }}">
        ...
        <li class="nav-item" role="presentation">
            <a class="nav-link active" href="{{ route('admin.signature.logs.verification') }}">
    ...
    <div class="filter-card">
        <form method="GET" action="{{ route('admin.signature.logs.verification') }}" id="filterForm">
            <div class="row g-3">
                ...
                <div class="col-lg-2 col-md-6">
                    ...
                    <select name="user_type" class="form-select form-select-sm">
                        ...
                        <option value="authenticated" {{ $userType == 'authenticated' ? 'selected' : '' }}>Authenticated</option>
                        <option value="anonymous" {{ $userType == 'anonymous' ? 'selected' : '' }}>Anonymous</option>
                    </select>
                </div>
                ...
                <div class="col-lg-12">
                    <div class="d-flex gap-2 justify-content-end">
                        ...
                        <a href="{{ route('admin.signature.logs.verification') }}" class="btn btn-light btn-sm">
                ...
                <ul class="timeline">
                    @foreach($logs as $log)
                        <li class="timeline-item">
                            <!-- Timeline Icon -->
                            <div class="timeline-icon" style="background: {{ $log->is_valid ? '#28a745' : '#dc3545' }}; color: white;">
                                <i class="fas fa-{{ $log->is_valid ? 'shield-alt' : 'exclamation-triangle' }}"></i>
                            </div>

                            <!-- Timeline Card -->
                            <div class="timeline-card {{ $log->is_valid ? 'valid' : 'invalid' }}">
                                <div class="timeline-header">
                                    <div>
                                        <h6 class="timeline-title">
                                            Signature Verification {{ $log->is_valid ? 'Successful' : 'Failed' }}
                                        </h6>
                                        <p class="mb-0 small">
                                            <span class="badge bg-{{ $log->is_valid ? 'success' : 'danger' }}">
                                                {{ $log->result_label }}
                                            </span>
                                        </p>
                                    </div>
                                    <div class="timeline-time">
                                        ...
                                        <br>
                                        <small class="text-muted">{{ $log->verified_at->format('d M Y') }}</small>
                                    </div>
                                </div>
                                ...
                                <div class="timeline-meta">
                                    ...
                                    <span class="meta-item">
                                        <span class="badge-method {{ strtolower($log->method_label) }}">
                                            <i class="fas fa-{{
                                                $log->method_label == 'Token' ? 'key' :
                                                ($log->method_label == 'URL' ? 'link' :
                                                ($log->method_label == 'QR Code' ? 'qrcode' : 'fingerprint'))
                                            }}"></i>
                                            {{ $log->method_label }}
                                        </span>
                                    </span>
                                ...
                                <!-- View Details Button -->
                                <div class="mt-3">
                                    <button class="btn btn-sm btn-outline-primary" onclick="viewLogDetails({{ $log->id }}, 'verification')">
        ...
        <div class="card border-0 shadow-sm">
            <div class="card-body">
                <div class="empty-state">
                    ...
                    <a href="{{ route('admin.signature.logs.verification') }}" class="btn btn-primary mt-3">
