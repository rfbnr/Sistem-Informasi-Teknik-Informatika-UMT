{{-- resources/views/digital-signature/admin/reports/qr-codes.blade.php --}}
@extends('digital-signature.layouts.app')

@section('title', 'QR Code Analytics Report')
...
<div class="main-content">
    ...
    <div class="row mb-4">
        <div class="col-lg-3 col-md-6 mb-3">
            <div class="card">
                <div class="card-body text-center">
                    <div class="h2 text-primary">{{ $analytics['total_codes'] }}</div>
        ...
        <div class="col-lg-3 col-md-6 mb-3">
            <div class="card">
                <div class="card-body text-center">
                    <div class="h2 text-success">{{ number_format($analytics['total_scans']) }}</div>
        ...
        <div class="col-lg-3 col-md-6 mb-3">
            <div class="card">
                <div class="card-body text-center">
                    <div class="h2 text-info">{{ $analytics['avg_scans_per_code'] }}</div>
        ...
        <div class="col-lg-3 col-md-6 mb-3">
            <div class="card">
                <div class="card-body text-center">
                    <div class="h2 text-warning">{{ $analytics['never_scanned'] }}</div>
    ...
    @if($analytics['most_scanned']->count() > 0)
    <div class="row mb-4">
        <div class="col-12">
            <div class="card">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if($qr->documentSignature && $qr->documentSignature->approvalRequest)
                                            {{ Str::limit($qr->documentSignature->approvalRequest->document_name, 40) }}
                                        @else
                                            <span class="text-muted">N/A</span>
                                        @endif
                                    </td>
                                    <td>
                                        <small class="text-muted">{{ $qr->created_at->format('d M Y') }}</small>
                                    </td>
                                    <td class="text-center">
                                        <span class="badge bg-primary fs-6">{{ $qr->access_count }}</span>
                                    </td>
                                    <td>
                                        @if($qr->last_accessed_at)
                                            <small class="text-muted">
                                                {{ $qr->last_accessed_at->diffForHumans() }}
                                            </small>
                                        @else
    ...
    <div class="card">
        <div class="card-header bg-primary text-white">
            <h5 class="mb-0">
                ...
                All Verification Codes ({{ $qrCodes->total() }})
            </h5>
        </div>
        <div class="card-body">
            @if($qrCodes->count() > 0)
                <div class="table-responsive">
                    <table class="table table-hover align-middle" id="qrCodesTable">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>
                                    <code class="bg-light p-1 rounded">{{ $qr->short_code }}</code>
                                </td>
                                <td>
                                    @if($qr->documentSignature && $qr->documentSignature->approvalRequest)
                                ...
                                <td>
                                    @if($qr->documentSignature && $qr->documentSignature->approvalRequest && $qr->documentSignature->approvalRequest->user)
                                        <div>{{ $qr->documentSignature->approvalRequest->user->name }}</div>
                                        <small class="text-muted">{{ $qr->documentSignature->approvalRequest->user->NIM }}</small>
                                    @else
                                        <span class="text-muted">Unknown</span>
                                    @endif
                                </td>
                                <td>
                                    <small class="text-muted">{{ $qr->created_at->format('d M Y H:i') }}</small>
                                </td>
                                <td>
                                    <small class="text-muted">
                                        {{ $qr->expires_at->format('d M Y') }}
                                        @if($qr->expires_at < now())
                                ...
                                <td class="text-center">
                                    @if($qr->access_count > 0)
                                        <span class="badge bg-primary">{{ $qr->access_count }}</span>
                                    @else
                                ...
                                <td>
                                    @if($qr->expires_at > now())
                                        ...
                                        <br>
                                        <small class="text-muted">
                                            {{ $qr->expires_at->diffInDays(now()) }} days left
                                        </small>
                                    @else
                                        <span class="badge bg-danger">Expired</span>
                                        <br>
                                        <small class="text-muted">
                                            {{ now()->diffInDays($qr->expires_at) }} days ago
                                        </small>
                                    @endif
                                </td>
                                <td>
                                    <div class="btn-group btn-group-sm">
                                        <button class="btn btn-outline-info"
                                                onclick="viewQRDetails('{{ $qr->short_code }}', {{ $qr->id }})"
                                                title="View Details">
